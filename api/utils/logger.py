...
async def api_logger(request: Request, response=None, error=None):
    ...
    email = user.sign_email.split("@") if user and user.sign_email else None
    user_log = dict(
        client=request.state.ip,
        user=user.sign_id if user and user.sign_id else None,
        email="**" + email[0][2:-1] + "*@" + email[1] if user and user.sign_email else None,
    )
